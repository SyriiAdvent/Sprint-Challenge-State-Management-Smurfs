...
- [X] What problem does the context API help solve?
  - Being another form of state managment. It helps us move our state/data around our application without being bogged down with prop drilling
- [X] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
- [X] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  - Application state is our global state, the current snapshot of the entire app. Where as a component state is just that, the components current status. 
  sometimes a component can just handle its own state, dont need redux/context to hold a tiny components state
- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  - thunk is a "Middleware" tool, Before a actionCreator initializes a action thunk allows us to run Asyncchronous code before hand to allow promises used with redux.
- [X] What is your favorite state management system you've learned and this sprint? Please explain why!
  - Despite all the commontion about "Redux shouldn't be used everywhere" I personally dont care, its so fun to use. I'll definatly be using it every chance i get.
